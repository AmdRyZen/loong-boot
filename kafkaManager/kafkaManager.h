...
public:
    ...
    void initialize(const std::string& brokers, const std::string& groupId = "default_group")
    {
        ...
        // 优化生产者配置
        rd_kafka_conf_set(producer_conf_, "queue.buffering.max.messages", "1000000", errStr, sizeof(errStr)); // 增加队列大小
        rd_kafka_conf_set(producer_conf_, "queue.buffering.max.kbytes", "2097152", errStr, sizeof(errStr)); // 2GB 队列容量
        rd_kafka_conf_set(producer_conf_, "batch.size", "1048576", errStr, sizeof(errStr)); // 1MB 批次大小
        rd_kafka_conf_set(producer_conf_, "linger.ms", "5", errStr, sizeof(errStr)); // 等待 5ms 批量发送
        rd_kafka_conf_set(producer_conf_, "compression.type", "snappy", errStr, sizeof(errStr)); // 启用压缩
    ...
    rd_kafka_topic_t* getTopic(const std::string& topicName)
    {
        ...
        if (!initialized_ || destroyed_)
            throw std::runtime_error("KafkaManager not initialized or destroyed");
        if (topicName.empty()) {
            std::cerr << "Error: Attempted to get empty topic name." << std::endl;
...
private:
    KafkaManager() : producer_conf_(nullptr), producer_(nullptr), consumer_conf_(nullptr), initialized_(false), destroyed_(false) {}
    ~KafkaManager()
    {
        stop();
    }
        ...
        pollThread_ = std::thread([this]() {
            while (running_) {
                try {
                    ...
                    std::cerr << "Exception in poll thread: " << e.what() << std::endl;
                }
